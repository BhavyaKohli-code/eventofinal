...
export default function DecoratorsScreen() {
  ...
  const [unavailablePackages, setUnavailablePackages] = useState([]);
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const response = await fetch('http://65.0.135.159:5000/api/rental-cars/packages');
    ...
    const fetchUnavailablePackages = async () => {
      try {
        const response = await fetch('http://65.0.135.159:5000/api2/package-availability');
        const data = await response.json();
        setUnavailablePackages(data);
      } catch (error) {
        console.error('Error fetching unavailable packages:', error);
      }
    };

    Promise.all([fetchPackages(), fetchUnavailablePackages()]).finally(() => setLoading(false));
  }, []);
